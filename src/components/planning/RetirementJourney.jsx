...
export default function RetirementJourney() {
  ...
  return (
    <section className="py-16 md:py-24 relative overflow-hidden bg-[#FAF8F5] border-y border-[#E7D7B5]">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[#C89B3C]/10 rounded-full blur-[140px] pointer-events-none" />

      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Heading */}
        <div className="text-center space-y-3 max-w-3xl mx-auto mb-10 sm:mb-14">
          <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-[#FAF8F5] border border-[#E7D7B5] text-[#B8860B] text-xs font-semibold uppercase tracking-widest font-sora shadow-xs">
            <Compass className="w-3.5 h-3.5" />
            <span>RETIREMENT ROADMAP</span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-serif-luxury font-bold text-[#0F172A] leading-tight">
        ...
        <motion.div
          ...
          className="relative h-[620px] sm:h-[680px] max-w-4xl mx-auto rounded-3xl p-6 sm:p-8 bg-white border border-[#E7D7B5] shadow-xl flex flex-col justify-between overflow-hidden"
        >
          ...
          <svg className="absolute inset-0 w-full h-full fill-none pointer-events-none" viewBox="0 0 500 620">
            ...
            {[
              ...
              <g key={i}>
                <circle cx={pt.x} cy={pt.y} r="8" fill="#C89B3C" opacity="0.25" />
                <circle cx={pt.x} cy={pt.y} r="4.5" fill="#C89B3C" stroke="#FFFFFF" strokeWidth="2" />
              </g>
            ))}
          </svg>
          ...
          {milestones.map((ms, index) => {
            ...
            return (
              ...
              >
                <div className="p-3 sm:p-4 rounded-2xl bg-[#FAF8F5] border border-[#E7D7B5] shadow-sm hover:shadow-md hover:border-[#C89B3C] transition-all duration-300 flex items-center space-x-3 text-left">
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white border border-[#C89B3C]/40 text-[#C89B3C] group-hover:bg-[#C89B3C] group-hover:text-white flex items-center justify-center shrink-0 transition-colors shadow-xs">
                    <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
                  </div>
                  <div>
                    <h4 className="text-xs sm:text-sm font-serif-luxury font-bold text-[#0F172A] group-hover:text-[#B8860B] transition-colors leading-tight">
                      {ms.name}
                    </h4>
