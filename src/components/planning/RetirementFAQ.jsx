    ...
    <section className="py-16 sm:py-24 bg-white relative overflow-hidden border-t border-[#E7D7B5]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center space-y-3 mb-12">
          <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-[#FAF8F5] border border-[#E7D7B5] text-[#B8860B] text-xs font-semibold uppercase tracking-widest font-sora">
          ...
          {faqs.map((faq, idx) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl border border-[#E7D7B5] bg-[#FAF8F5] overflow-hidden shadow-xs"
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? -1 : idx)}
                  className="w-full p-5 sm:p-6 text-left flex items-center justify-between gap-4 font-serif-luxury font-bold text-base sm:text-lg text-[#0F172A] hover:text-[#B8860B] transition-colors cursor-pointer"
                >
                  <span>{faq.q}</span>
                  <div className={`w-8 h-8 rounded-full bg-white border border-[#E7D7B5] flex items-center justify-center text-[#C89B3C] shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 bg-[#C89B3C] text-white' : ''}`}>
                ...
                <AnimatePresence>
                  {isOpen && (
                    ...
                    >
                      <div className="p-5 sm:p-6 pt-0 border-t border-[#E7D7B5]/60 text-xs sm:text-sm text-[#475569] leading-relaxed font-sans text-left">
                        {faq.a}
                      </div>
                    </motion.div>
                  )}
