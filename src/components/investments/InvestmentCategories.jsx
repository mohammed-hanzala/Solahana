...
export default function InvestmentCategories() {
  ...
  return (
    <section id="investment-categories" className="py-16 sm:py-24 bg-white relative overflow-hidden border-t border-[#C89A4B]/15">
          ...
          {solutions.map((sol, idx) => {
            ...
            return (
              <motion.div
                ...
                className="p-6 rounded-[22px] bg-[#FFFDF8] border border-[#C89A4B]/20 hover:border-[#C89A4B] shadow-[0_10px_30px_rgba(200,154,75,0.06)] hover:shadow-lg transition-all duration-300 text-left flex flex-col justify-between group cursor-pointer"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-11 h-11 rounded-2xl bg-white border border-[#C89A4B]/30 text-[#C89A4B] group-hover:bg-[#C89A4B] group-hover:text-white flex items-center justify-center transition-colors shadow-xs">
                      <Icon className="w-5 h-5" />
                    </div>
                    <ArrowUpRight className="w-4 h-4 text-[#C89A4B] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                  </div>

                  <h3 className="text-base font-serif-luxury font-bold text-[#0F172A] group-hover:text-[#C89A4B] transition-colors mb-2">
                    {sol.title}
                  </h3>

                  <p className="text-xs text-[#475569] font-sans leading-relaxed line-clamp-3">
                    {sol.desc}
                  </p>
                </div>

                <div className="mt-4 pt-3 border-t border-[#C89A4B]/15 text-[11px] font-semibold text-[#9A7326] flex items-center justify-between">
